...
export async function lookupWeatherZone(lat: number, lon: number): Promise<{ weather_zone_id: number }> {
...
// ---------------------------------------------------------------------
// Användarstyrda funktioner (Auth / RLS)
// ---------------------------------------------------------------------
...
export async function updateMyProfile(update: { display_name?: string; avatar_url?: string }): Promise<UserProfile> {
...
export async function uploadDiscovery(params: {
  ...
  const { error: uploadError } = await supabase.storage.from("discoveries").upload(path, params.file, {
