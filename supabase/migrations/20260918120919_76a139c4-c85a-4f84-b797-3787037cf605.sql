...
BEGIN
  ...
  SELECT z.center_lon,
         z.center_lat,
         v_date,
         greatest(0, least(1, (0.50 * coalesce(w.moisture_score, 0) + 0.25 * soil.v + 0.25 * forest.v) * v_season)),
         soil.v,
         forest.v,
  ...
  CROSS JOIN LATERAL (
    SELECT 0.30 + 0.70 * ((hashtext(z.id::text || ':soil:' || v_slug) & 1023) / 1023.0) AS v
  ) soil
  CROSS JOIN LATERAL (
    SELECT 0.20 + 0.80 * z.forest_cover AS v
  ) forest
  WHERE z.is_active
...
RETURNS TABLE(lon double precision, lat double precision, obs_date date, moisture_score double precision, precip_7d_sum double precision, precip_10d_sum double precision, temp_mean double precision)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
  ...
  FROM public.weather_zones z
  JOIN public.weather_observations w
    ON w.weather_zone_id = z.weather_sample_id
   AND w.obs_date = coalesce(p_obs_date, (SELECT max(o.obs_date) FROM public.weather_observations o))
  WHERE z.is_active
    AND coalesce(w.moisture_score, 0) >= coalesce(p_min_score, 0)
    AND z.center_lon BETWEEN p_min_lon AND p_max_lon
    AND z.center_lat BETWEEN p_min_lat AND p_max_lat
    AND (least(greatest(coalesce(p_step, 1), 1), 20) = 1
         OR (z.grid_x % least(greatest(coalesce(p_step, 1), 1), 20) = 0
             AND z.grid_y % least(greatest(coalesce(p_step, 1), 1), 20) = 0))
  ORDER BY w.moisture_score DESC NULLS LAST
  LIMIT least(greatest(coalesce(p_limit, 2000), 1), 25000);
$function$;

DROP FUNCTION IF EXISTS public.get_moisture_layer(double precision, double precision, double precision, double precision, date, integer);
DROP FUNCTION IF EXISTS public.get_moisture_layer(double precision, double precision, double precision, double precision, date, double precision, integer);
DROP FUNCTION IF EXISTS public.get_predictions(bigint, double precision, double precision, double precision, double precision, date, double precision, integer);
...
GRANT EXECUTE ON FUNCTION public.get_moisture_layer(double precision, double precision, double precision, double precision, date, double precision, integer, integer) TO anon, authenticated, service_role;