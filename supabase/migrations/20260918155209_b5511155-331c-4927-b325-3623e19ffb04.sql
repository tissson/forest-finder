...
BEGIN
  ...
  v_level := CASE WHEN z <= 5 THEN 4 WHEN z <= 7 THEN 2 ELSE 0 END;
  ...
  IF v_level > 0 THEN
    ...
    FROM (
      SELECT ST_AsMVTGeom(ST_Transform(ST_Centroid(b.geom), 3857), v_env, 4096, 64, true) AS geom,
             b.zone_id,
             greatest(0, least(1,
  ...
  ELSE
    SELECT ST_AsMVT(t, 'predictions', 4096, 'geom') INTO v_tile
    FROM (
      SELECT ST_AsMVTGeom(ST_Transform(ST_Centroid(wz.geom), 3857), v_env, 4096, 64, true) AS geom,
             wz.id AS zone_id,
             greatest(0, least(1,
               (0.50 * coalesce(w.moisture_score, 0)
