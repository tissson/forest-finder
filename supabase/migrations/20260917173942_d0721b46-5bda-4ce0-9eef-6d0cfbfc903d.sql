...
create policy "Väderdata är publik" on public.weather_observations for select to anon, authenticated using (true);
...
create index if not exists predictions_lookup_idx on public.predictions (species_id, obs_date, score_total desc);
...
create policy "Prognoser är publika" on public.predictions for select to anon, authenticated using (true);
...
create policy "Läs egen profil" on public.user_profiles for select to authenticated using (auth.uid() = user_id);
drop policy if exists "Skapa egen profil" on public.user_profiles;
create policy "Skapa egen profil" on public.user_profiles for insert to authenticated with check (auth.uid() = user_id);
drop policy if exists "Uppdatera egen profil" on public.user_profiles;
create policy "Uppdatera egen profil" on public.user_profiles for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create index if not exists user_discoveries_user_idx on public.user_discoveries (user_id, created_at desc);
...
create policy "Läs egna fynd" on public.user_discoveries for select to authenticated using (auth.uid() = user_id);
drop policy if exists "Skapa egna fynd" on public.user_discoveries;
create policy "Skapa egna fynd" on public.user_discoveries for insert to authenticated with check (auth.uid() = user_id);
drop policy if exists "Radera egna fynd" on public.user_discoveries;
create policy "Radera egna fynd" on public.user_discoveries for delete to authenticated using (auth.uid() = user_id);
...
create policy "Läs egna badges" on public.user_badges for select to authenticated using (auth.uid() = user_id);
...
create policy "Utmaningar är publika" on public.weekly_challenges for select to anon, authenticated using (true);
...
begin
  ...
  v_date := coalesce(p_obs_date, (select max(p.obs_date) from public.predictions p where p.species_id = p_species_id));
  return query
  select z.center_lon, z.center_lat, p.obs_date, p.score_total, p.score_soil, p.score_forest, p.score_weather
...
set search_path = public
as $$
  select z.center_lon, z.center_lat, w.obs_date, w.moisture_score, w.precip_7d_sum, w.precip_10d_sum, w.temp_mean
...
begin
  ...
  where species_id = p_species_id and current_date between start_date and end_date limit 1;
