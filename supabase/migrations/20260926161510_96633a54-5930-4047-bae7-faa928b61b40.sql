ALTER TABLE public.weather_zones ADD COLUMN IF NOT EXISTS soil_wetness double precision;
...
DECLARE
  ...
  v_moisture_optimum double precision; v_moisture_tolerance double precision;
  v_forest_optimum double precision; v_forest_tolerance double precision;
...
BEGIN
  SELECT s.tier, s.season_start, s.season_end, hp.moisture_optimum, hp.moisture_tolerance, hp.forest_optimum, hp.forest_tolerance
    INTO v_tier, v_start, v_end, v_moisture_optimum, v_moisture_tolerance, v_forest_optimum, v_forest_tolerance
  FROM public.species s JOIN public.species_habitat_profiles hp ON hp.species_id = s.id
  ...
  SELECT ST_AsMVT(t, 'predictions', 4096, 'geom') INTO v_tile FROM (
    ...
    FROM (
      SELECT wz.id AS zone_id, wz.geom, m.moisture,
        greatest(0, least(1, (
            0.53 * exp(-power((m.moisture - v_moisture_optimum) / v_moisture_tolerance, 2))
