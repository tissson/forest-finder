  ...
  FROM (
    ...
    FROM (
      SELECT min(wz.id) AS zone_id,
             greatest(0, least(1,
               (0.50 * avg(coalesce(w.moisture_score, 0))
                + 0.25 * avg(0.30 + 0.70 * ((hashtext(wz.id::text || ':soil:' || v_slug) & 1023) / 1023.0))
                + 0.25 * avg(0.20 + 0.80 * wz.forest_cover)) * v_season))::double precision AS score,
